...
export default forwardRef(function FillInfo(props, ref) {
  ...
  const exportClick = () => {
    ...
    utils.book_append_sheet(wb, ws, "数据详情");
    // 写文件(book,xlsx文件名称)
    writeFile(wb, title + "--详情.xlsx");
  };
  return (
    <Modal
      title={title}
      ...
      <Table dataSource={fillData} bordered rowKey="content" pagination={false}>
        <Column title="提交答卷时间" align="center" dataIndex="time" key="time" width={180} />
        <Column title="答案文本" align="center" dataIndex="content" key="content" />
      </Table>
    </Modal>
  );
});
